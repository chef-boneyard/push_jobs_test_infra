...
resource "aws_vpc" "chef_push_testing" {
  cidr_block           = "${var.vpc_cidr}"
  enable_dns_hostnames = true

  tags {
    Name      = "Chef Push Jobs Test VPC"
    X-Contact = "Thom May <tmay@chef.io>"
  }
}

resource "aws_internet_gateway" "default" {
  vpc_id = "${aws_vpc.chef_push_testing.id}"
}
...
resource "null_resource" "build_knife_config" {
  ...
  provisioner "local-exec" {
    command = "chef install Policyfile.rb && chef push base Policyfile.lock.json"
  }
}
...
resource "aws_instance" "client" {
  ...
  provisioner "chef" {
    ...
    node_name               = "push_client_${count.index + 1}"
